...
Host base: block/buzz `813bbd14121edacc6cb4733301a3af12131aa10e`.
...
Search is a literal case-insensitive substring of root **body**, not only the
240-character whitespace-collapsed preview. Empty titles show “Untitled thread”.
Only roots with canonical depth-zero metadata are eligible: legacy messages
without thread metadata are conservatively excluded, not guessed to be roots.
...
switch, and unmount. There is no persistent status or cache. Changes can take
...
```json
[{"kinds":[9],"#h":["channel-uuid"],"thread_flags":{"mode":"active","q":"","limit":50,"cursor":null}}]
```
